...
from ml import create_agent, get_answer_gpt, query_agent
...
def get_langchain_agent(dataset_id: str):
    try:
        df = load_dataframe(get_data_path(dataset_id))
        return create_agent(df)
    except Exception:
        return None


async def load_dataset_info_from_db(dataset_id: str) -> dict | None:
    dataset_info = await db.hypotheses.find_one(
        {"meta.id": dataset_id}, projection={"_id": False}
    )
    for hypothesis in dataset_info["hypotheses"]:
        hypothesis["title"] = hypothesis["title"].removeprefix("Гипотеза:")
    return dataset_info
...
async def save_dataset_info_to_db(hypothesis: dict) -> None:
    await db.hypotheses.insert_one(hypothesis)
...
async def upload_data(file: UploadFile = File(...)):
    try:
        ...
        hypotheses = await get_hypotheses(
            get_data_path(dataset_id), dataset_id=dataset_id, filename=file.filename
        )
        await save_dataset_info_to_db(hypotheses)
        return JSONResponse(
...
@app.get("/dataset")
async def hypotheses_list(dataset_id: str):
    if hypotheses := await load_dataset_info_from_db(dataset_id):
        if len(hypotheses["hypotheses"]) > 1:
            hypotheses["hypotheses"] = hypotheses["hypotheses"][:-1]
        return hypotheses
    return HTTPException(status_code=404, detail="Dataset was not uploaded")


@app.get("/dataset_chat")
async def hypotheses_list(dataset_id: str, user_query: str):
    if dataset_agent := get_langchain_agent(dataset_id):
        return await query_agent(agent=dataset_agent, query=user_query)
    return HTTPException(status_code=404, detail="Dataset was not uploaded")


@app.get("/all_datasets")
async def datasets_list(p: int = 1, limit: int = None):
    if limit:
        all = await db.hypotheses.find(
            projection={
                "meta": True,
                "_id": False,
            }
        ).sort("meta.upload_ts", -1).skip(limit * (p - 1)).limit(limit).to_list(length=limit)
    else:
        all = await db.hypotheses.find(
            projection={
                "meta": True,
        ...
        ).sort("meta.upload_ts", -1).to_list(length=5_000)
    
    for hypothesis in all:
        hypothesis["meta"]["upload_ts"] = (hypothesis["meta"]["upload_ts"] + datetime.timedelta(hours=3)).strftime("%d.%m.%Y %H:%M:%S")
